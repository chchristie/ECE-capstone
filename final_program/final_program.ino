...
void loop() {
  ...
  if (intFlag) {
    ...
    if (intCounter <= 10) {
      accelBuffX1[intCounter - 1] = myIMU.readRawAccelX() * 61 / 1000;
      accelBuffY1[intCounter - 1] = myIMU.readRawAccelY() * 61 / 1000;
      accelBuffZ1[intCounter - 1] = myIMU.readRawAccelZ() * 61 / 1000;
    } else {
      accelBuffX2[intCounter - 11] = myIMU.readRawAccelX() * 61 / 1000;
      accelBuffY2[intCounter - 11] = myIMU.readRawAccelY() * 61 / 1000;
      accelBuffZ2[intCounter - 11] = myIMU.readRawAccelZ() * 61 / 1000;
    }
...
void sendAccelerometerBLE() {
  ...
  int16_t magnitude = sqrt(x * x + y * y + z * z);
